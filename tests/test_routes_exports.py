...
async def test_export_books_save_creates_exports_dir(client, conn, data_root: Path):
    conn.execute(
        "INSERT INTO book (slug, title, format, source_path, view_path)"
        " VALUES ('mkdir-book', 'Mkdir Book', 'txt', '/x', '/x')"
    )
    exports_dir = data_root / "exports"
    ...
    assert list(exports_dir.glob("books-*.csv"))
